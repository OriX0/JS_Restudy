...
一个表单中的任何元素都可以通过`form.elements`来获取
...
可以将 `form.elements.login` 写成 `form.login`



一个小问题：如果我们访问一个元素，然后修改它的 `name`，之后它仍然可以被通过旧的 `name`访问到（当然也能通过新的 `name` 访问）
...
通过 `input.value`访问input的值

通过`textarea.value` 访问textarea的值  不能通过innerHTML

通过`input.checked`访问是否选中

#### select和 option 下拉列表框
...
##### 如果是多选的情况 即具有 `multiple` 特性

可以通过 `select.options` 获取选中的集合
...
`defaultSelected` 设置的是 HTML-特性（attribute），我们可以使用 `option.getAttribute('selected')` 来获得。

`selected` —— 无论这个 `option` 是否被选则，它都很重要。

**通常，这两个值都设置为 `true`，或者都不设置（与设置为 `false` 是一样的）。**
